...
def download_bench_photos(attTable, photoFeatureClass, visitFeatureClass, dataPhotoLocation, originalsLocation):
    # Cursor for attachment table
    att_cursor = da.SearchCursor(attTable, ['DATA', 'ATT_NAME', 'ATTACHMENTID', 'REL_GLOBALID'])
    for item in att_cursor:
        fk_photo = item[3]  # Get global id for corresponding row in photo feature class
        # Cursor for data table
        photo_cursor = da.SearchCursor(photoFeatureClass, field_names = ['BMPhotos', 'parentglobalid'], where_clause = "GLOBALID = " + "'" + fk_photo + "'")
        ...
        visit_cursor = da.SearchCursor(visitFeatureClass, field_names = ['LakeID', 'SampleDate'], where_clause = "GLOBALID = " + "'" + fk_visit + "'")  # Get global id for corresponding row in visit feature class
        for row in visit_cursor:
            lake = row[0]
            time = datetime.strftime(row[1], "%Y%m%d")
            time_folder = datetime.strftime(row[1], "%Y_%m_%d")
